...
void BMP388::writeRegister(int address, int value) const
{
...
void BMP388::readRegister(int address) const
{
  Wire.beginTransmission(I2Caddress);
  Wire.write(address);
  Wire.endTransmission();
...
uint8_t BMP388::readByte(uint8_t address)
{
  Wire.beginTransmission(I2Caddress);         
	Wire.write(address);                  
...
void BMP388::readBytes(uint8_t address, uint8_t* data, uint16_t count)
{
  Wire.beginTransmission(I2Caddress);          
	Wire.write(address);                   
	Wire.endTransmission(false);
...
bool BMP388::getMeasurements(volatile float &temperature, volatile float &pressure, volatile float &altitude)
{
...
bool BMP388::getTemperatureAndPressure(volatile float& temperature, volatile float& pressure)	
{
	if (!dataReady())
	{	
		return false;
...
float BMP388::compensateTemperature(float uncompensatedTemperature)
{
...
float BMP388::compensatePressure(float uncompensatedPressure, float temperature)
{
